...
class FullyConnectedNeuralNet(nn.Module):
    def __init__(self, inputSize, numClasses, activation, nLayers, nUnitsList, dropout):
        """
        Initialize a FCC.
    ...
    def construct_network(self):
        ...
        """        
        layers = []
        inFeatures = self.inputSize
       
        for i in range(self.nLayers):
            layers += [nn.Linear(inFeatures, self.nUnitsList[i])]
            layers += [self.activation]
            layers += [nn.Dropout(self.dropout)]  
            inFeatures = self.nUnitsList[i]
        layers += [nn.Linear(inFeatures, self.numClasses)]
