...
def train(model, trainLoader, validLoader, device, nbEpochs, opt, learningRateExponent, optimParams=None):
    """
    ...
    :param (int) learningRateExponent: the value of the learning rate exponent (learning rate = 10 ** learningRateExponent)
    ...
    if optimParams == None:
        ...
        if opt == "Adam":
            optimizer = optim.Adam(model.parameters(), lr=learningRate, betas=(optimParams[0], optimParams[1]), eps=optimParams[2])
        elif opt == "ASGD":
            optimizer = optim.ASGD(model.parameters(), lr=learningRate, lambd=optimParams[0], alpha=optimParams[1], t0=optimParams[2])
        elif opt == "Adagrad":
            optimizer = optim.Adagrad(model.parameters(), lr=learningRate, lr_decay=optimParams[0], initial_accumulator_value=optimParams[1], eps=optimParams[2])
        elif opt == "RMSprop":
    ...
    while (not stop) and (epoch <= nbEpochs):  # loop over the dataset multiple times
        ...
        if PRINT:
            print("\tExecution time: {:.2f}s, Train accuracy: {:.2f}%, Val accuracy: {:.2f}%".format(executionTime, trainAcc, valAcc))
